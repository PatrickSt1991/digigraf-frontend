...
export default function FuneralDeceased() {
  ...
  const saveurl = dossierId ? `${endpoints.funeral}/${dossierId}` : endpoints.funeral;
      ...
      <div className="px-8 pb-8 max-w-8xl mx-auto space-y-6">
        ...
        {formLoading ? (
          <LoadingState
            ...
            message="Uitvaartgegevens worden geladen..."
          />
        ) : formError ? (
          <ErrorState
            ...
            <FormCard title="">
              ...
              <FormField label="Kist staan / dalen" required>
                {dropdownLoading.coffinservice ? (
                  <div className="text-sm text-gray-500">Opties worden geladen...</div>
                ) : dropdownErrors.coffinservice ? (
                  <div className="text-sm text-red-600">{dropdownErrors.coffinservice}</div>
                ) : (
                  <select
                    name="coffinUpDown"
              ...
              <FormField label="Asbestemming">
                {dropdownLoading.ashdestinations ? (
                  <div className="text-sm text-gray-500">Asbestemmingen worden geladen...</div>
                ) : dropdownErrors.ashdestinations ? (
                  <div className="text-sm text-red-600">{dropdownErrors.ashdestinations}</div>
                ) : (
                  <select
                    name="coffinUpDown"
              ...
              <FormField label="Graf type">
                {dropdownLoading.gravetypes ? (
                  <div className="text-sm text-gray-500">Graftypes worden geladen...</div>
                ) : dropdownErrors.gravetypes ? (
                  <div className="text-sm text-red-600">{dropdownErrors.gravetypes}</div>
                ) : (
                  <select
                    name="coffinUpDown"
